...
/** L'heure locale d'un instant, au format HH:MM (pour pré-remplir un champ <input type="time">). */
export function heureLocale(instant: Date, fuseau: string) {
  return new Intl.DateTimeFormat("en-GB", { timeZone: fuseau, hour: "2-digit", minute: "2-digit", hourCycle: "h23" }).format(
    instant,
  );
}
